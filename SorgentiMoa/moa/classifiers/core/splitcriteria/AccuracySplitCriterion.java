 ...
 * @version $Revision: 7 $
 */
public class AccuracySplitCriterion extends AbstractOptionHandler implements
        SplitCriterion {
    ...
    public double getMeritOfSplit(double[] preSplitDist,
        ...
        double accuracySplit = 0.0;
        
        for (int i = 0; i < postSplitDists.length; i++) {
            accuracySplit += (distWeights[i] / totalWeight)
                    * computeErrorBound(postSplitDists[i], distWeights[i]);
        }
        
        return 1.0 - accuracySplit;
    }
    ...
    public static double computeErrorBound(double[] dist, double distSumOfWeights) {
        
         Arrays.sort(dist);

            //get minimum p
         double p = dist[0] / distSumOfWeights;
        
        
        return p;
    }

    public static double computeErrorBound(double[] dist) {
        return computeErrorBound(dist, Utils.sum(dist));
    }
