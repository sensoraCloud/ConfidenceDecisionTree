 ...
 */
public class McDiarmidBoundTree extends TransparentHoeffdingTree {

    public McDiarmidBoundTree(int seed) {
        super(seed);
    }

    public McDiarmidBoundTree() {
        super();
    ...
    public static double computeHoeffdingBound(double range, double confidence,
        ...
        double CNK = 6 * (K * (Math.log(2.71 * n) / Math.log(2)) + (Math.log(2 * n) / Math.log(2))) + 2 * (Math.log(K) / Math.log(2));

        double bound = CNK * (Math.sqrt((Math.log(1.0 / GS.delta_McD)) / (2.0 * n)));
